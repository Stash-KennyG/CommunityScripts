...
// @match        https://stash.shannabower.com/performers*
// @grant        none
// @run-at       document-end
...
(function () {
    ...
    // Highlight scenes that have no performer source block
    // A "performer source" is defined as an .mt-2 that contains an .entity-name
    // whose text starts with "Performer:". If a scene card (.mt-3.search-item)
    // has no such block, we outline the entire card in red – but only when the
    // Save button is visible (i.e. the card is in an editable state).
    function highlightScenesMissingPerformerSource() {
        const searchItems = document.querySelectorAll('div.mt-3.search-item');
        searchItems.forEach(searchItem => {
            // Only apply borders when the *Save* button for this card is visible.
            // First find the bottom row container, then look for a primary button
            // whose label is exactly \"Save\" so we don't match other buttons.
            const saveRow = searchItem.querySelector(
                '.row.no-gutters.mt-2.align-items-center.justify-content-end.mt-auto.w-100.d-flex'
            );
            let saveButton = null;
            if (saveRow) {
                const buttons = Array.from(saveRow.querySelectorAll('button.btn.btn-primary'));
                saveButton = buttons.find(btn => btn.textContent.trim() === 'Save') || null;
            }
            const hasVisibleSave = !!(saveButton && saveButton.offsetParent !== null);

            if (!hasVisibleSave) {
                // Clear any previous outline if we're not in an editable state
                if (searchItem.style.outline) {
                    searchItem.style.outline = '';
                    searchItem.style.outlineOffset = '';
                }
                return;
            }

            const performerBlocks = searchItem.querySelectorAll('.mt-2 .entity-name');
            const hasPerformerSource = Array.from(performerBlocks).some(el =>
                el.textContent.trim().startsWith('Performer:')
            );

            if (!hasPerformerSource) {
                searchItem.style.outline = '2px solid #B71C1C'; // crimson red outline
                searchItem.style.outlineOffset = '2px';
            } else {
                // Clear outline if it was previously applied
                if (searchItem.style.outlineColor === 'rgb(183, 28, 28)' || searchItem.style.outline.includes('#B71C1C')) {
                    searchItem.style.outline = '';
                    searchItem.style.outlineOffset = '';
                }
            }
    ...
    function runAllHighlights() {
        ...
        highlightScenesMissingPerformerSource();
    }
