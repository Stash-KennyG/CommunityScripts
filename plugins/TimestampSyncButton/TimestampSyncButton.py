                ...
                if res.status_code == 200:
                    ...
                    if settings["createMarkers"]:
                        ...
                        for m in data["markers"]:
                            ...
                            if marker["primary_tag"] and not settings["autoCreateTags"]:
                                if not tag_exists(marker["primary_tag"]):
                                    log.info(
                                        "UNMATCHED TAG (auto-create disabled): %s"
                                        % (marker["primary_tag"],)
                                    )
                                    previous = marker
                                    continue
...
if "TimestampSyncButton" in config["plugins"]:
    settings.update(config["plugins"]["TimestampSyncButton"])
